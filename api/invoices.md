...
### `iban` و `account_number`

هر دو از روی کارتی می‌آیند که فاکتور روی آن صادر شده و هر دو اختیاری‌اند — اگر فروشنده پرشان نکرده باشد `null` برمی‌گردند.

اگر صفحه‌ی پرداخت خودتان را می‌سازید و پرند، کنار شماره کارت نشانشان دهید: خریدار می‌تواند به‌جای کارت به کارت، **یک انتقال آنی با پل** بزند. مبلغی که باید بفرستد همان `payable_rial` است، بدون هیچ تغییری.

> [!NOTE]
> این برای فاکتور بزرگ فرق می‌کند. سقفی که فاکتور را تکه می‌کند سقف کارت است، نه سقف حساب — پس فاکتور چندتکه با یک انتقال هم پرداخت می‌شود.

تطبیق برای هر دو مسیر یکی است: سیستم روی مبلغ یکتا کار می‌کند، نه روی اینکه پول از کدام ریل آمده.
